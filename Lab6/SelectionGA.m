function ind = SelectionGA(f,X,Y)

%Inputs:
%
%       f: Evaluation function, use "@" before the name of the function
%       X: Matrix with input samples
%       Y: Output variable

%Outputs:
%
%       ind: Índices de las variables seleccionadas 1 se incluye, 0 se  excluye.

% SelectionGA is a modification of SpeedyGA, which is a vectorized implementation of a Simple Genetic Algorithm in Matlab
% Version 1.3
...
len=size(X,2);                    % The length of the genomes  
popSize=len * 2;               % The size of the population (must be an even number)
maxGens=100;              % The maximum number of generations allowed in a run
probCrossover=0.95;           % The probability of crossing over. 
...
ind = mean(pop)>0.2; %La variable se incluye si aparece en el 40% de los individuos finales. 
if verboseFlag
    figure(2)
    %set(gcf,'Color','w');
    hold off
